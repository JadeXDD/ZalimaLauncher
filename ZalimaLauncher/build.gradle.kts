...
val zalimaPackageName = "com.movtery.zalimalauncher"
...
android {
    namespace = zalimaPackageName
    compileSdk = 37

    signingConfigs {
        create("releaseBuild") {
            storeFile = file("zalima_launcher.jks")
            storePassword = getKeyFromLocal("STORE_PASSWORD", ".store_password.txt")
            keyAlias = "movtery_zalima"
            keyPassword = getKeyFromLocal("KEY_PASSWORD", ".key_password.txt")
        }
        create("debugBuild") {
            storeFile = file("zalima_launcher_debug.jks")
            storePassword = defaultStorePassword
            keyAlias = "movtery_zalima_debug"
            keyPassword = defaultKeyPassword
        }
    }

    defaultConfig {
        applicationId = zalimaPackageName
