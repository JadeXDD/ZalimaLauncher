...
import com.movtery.zalimalauncher.game.version.installed.VersionsManager.getZalimaVersionPath
...
import com.movtery.zalimalauncher.utils.string.getStringNotNull
import java.io.File
...
class VersionConfig(
    ...
    fun saveWithThrowable() {
        val zalimaVersionPath = getZalimaVersionPath(versionPath)
        val configFile = File(zalimaVersionPath, "version.config")
        if (!zalimaVersionPath.exists()) zalimaVersionPath.mkdirs()
    ...
    companion object CREATOR : Parcelable.Creator<VersionConfig> {
        ...
        fun parseConfig(versionPath: File): VersionConfig {
            val configFile = File(getZalimaVersionPath(versionPath), "version.config")
