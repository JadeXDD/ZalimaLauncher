...
package com.movtery.zalimalauncher.game.version.installed

import com.movtery.zalimalauncher.BuildKeys
import com.movtery.zalimalauncher.game.launch.LogName
import com.movtery.zalimalauncher.game.path.getVersionsHome
import com.movtery.zalimalauncher.game.version.installed.utils.parseJsonToVersionInfo
...
object VersionsManager {
     ...
     */
    fun getZalimaVersionPath(version: Version) = File(version.getVersionPath(), BuildKeys.LAUNCHER_IDENTIFIER)

    /**
     * @return 通过目录获取 Zalima 启动器版本标识文件夹
     */
    fun getZalimaVersionPath(folder: File) = File(folder, BuildKeys.LAUNCHER_IDENTIFIER)

    /**
     * @return 通过名称获取 Zalima 启动器版本标识文件夹
     */
    fun getZalimaVersionPath(name: String) = File(getVersionPath(name), BuildKeys.LAUNCHER_IDENTIFIER)

    /**
     * @return 游戏的上一次运行日志
     */
    fun getLatestLog(version: Version) = File(getZalimaVersionPath(version), LogName.GAME.fileName)
     ...
     * @return 通过目录获取 Zalima 启动器版本标识文件夹
     */
    fun getVersionIconFile(folder: File) = File(getZalimaVersionPath(folder), "VersionIcon.png")
