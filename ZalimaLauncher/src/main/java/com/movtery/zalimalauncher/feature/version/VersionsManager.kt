package com.movtery.zalimalauncher.feature.version

import android.content.Context
import com.movtery.zalimalauncher.InfoDistributor
import com.movtery.zalimalauncher.R
import com.movtery.zalimalauncher.event.single.RefreshVersionsEvent
import com.movtery.zalimalauncher.event.single.RefreshVersionsEvent.MODE.END
import com.movtery.zalimalauncher.event.single.RefreshVersionsEvent.MODE.START
...
import com.movtery.zalimalauncher.feature.version.utils.VersionInfoUtils
...
import com.movtery.zalimalauncher.utils.stringutils.SortStrings
...
object VersionsManager {
     ...
     * @see com.movtery.zalimalauncher.event.single.RefreshVersionsEvent
     */
    ...
    private fun processVersionFile(versionsHome: String, versionFile: File, refreshVersionInfo: Boolean) {
        if (versionFile.exists() && versionFile.isDirectory) {
            ...
            if (jsonFile.exists() && jsonFile.isFile) {
                isVersion = true
                val versionInfoFile = File(getZalimaVersionPath(versionFile), "VersionInfo.json")
    ...
    /**
     * @return 获取 Zalima 启动器版本标识文件夹
     */
    fun getZalimaVersionPath(version: Version) = File(version.getVersionPath(), InfoDistributor.LAUNCHER_NAME)

    /**
     * @return 通过目录获取 Zalima 启动器版本标识文件夹
     */
    fun getZalimaVersionPath(folder: File) = File(folder, InfoDistributor.LAUNCHER_NAME)

    /**
     * @return 通过名称获取 Zalima 启动器版本标识文件夹
     */
    fun getZalimaVersionPath(name: String) = File(getVersionPath(name), InfoDistributor.LAUNCHER_NAME)

    /**
     * @return 获取当前版本设置的图标
     */
    fun getVersionIconFile(version: Version) = File(getZalimaVersionPath(version), "VersionIcon.png")

    /**
     * @return 通过名称获取当前版本设置的图标
     */
    fun getVersionIconFile(name: String) = File(getZalimaVersionPath(name), "VersionIcon.png")
